...
import { news, pushInbox } from './ui/toast.js';
...
async function boot() {
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape' && modalStack.length && modalStack[modalStack.length - 1].dismissable !== false) closeModal();
  });
  ...
  openModal({
    ...
    body: `
      ...
      <div style="margin:12px 0">
        <div style="font-size:10px" class="c-dim">회사명</div>
        <input id="co-name" value="한별상사" maxlength="10"
          style="width:100%;margin-top:4px;padding:8px;font-family:inherit;font-size:14px;background:var(--paper-2);border:2px solid var(--ink);color:var(--ink)">
      </div>
      <div style="border-top:2px solid var(--paper-3);padding-top:10px;font-size:11px;line-height:2">
        <b>흐름</b><br>
        · 도시 맵에서 <b class="c-gold">금빛 건물</b>이 우리 회사입니다. 클릭하면 경영 모드로 들어갑니다.<br>
        · 다른 건물을 클릭하면 인수 대상 정보가 뜹니다. <b class="c-sky">협상단</b>을 파견하면 진행도가 차오릅니다.<br>
        · 협상 중에도 경영은 계속됩니다. 진행도 100%에서 <b>성공도만큼의 확률</b>로 인수가 결정됩니다.<br>
        · 인수가를 못 내면 인수는 불발됩니다. 은행 대출로 메울 수 있지만 원리금은 갚아야 합니다.<br>
        · <span class="c-dim">Space 일시정지 · Tab 모드 전환</span>
      </div>`,
